...
def login():
    ...
    cursor=conn.cursor()
    cursor.execute("select * from employee where USERNAME='"+e1.get()+"' and PASSWORD='"+e2.get()+"'")
    if(cursor.fetchone()):
        root.destroy()
        import empaft
...
Button(root,text="Login",fg="Black",font=("BROADWAY",10),command=login).grid(row=2,column=1)
root.mainloop()
