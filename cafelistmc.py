...
def showall():
    ...
    tree.pack()
    cursor.execute("select * from maincourse")
    f =cursor.fetchall()
    #fetch=[2,3,4,6,7,8,9]
    for data in f:
...
def ser(x):
    ...
    tree.pack()
    cursor.execute("select * from maincourse where SNo like '"+x+"%'")
    f =cursor.fetchall()
    #fetch=[2,3,4,6,7,8,9]
    for data in f:
        tree.insert('', 'end', values=(data))
...
first.pack(side="top")
Label(first,text="***Main Course***",fg="navyblue",bg="PeachPuff",font=("comic sans MS",50,"bold")).place(x=380,y=5)
